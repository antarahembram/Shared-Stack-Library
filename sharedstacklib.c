...
void shstackget(key_t mykey, int data_size,int stack_size)
{
  ...
  int shmid=shmget(key,sizeof(struct stack_desc),IPC_CREAT | 0777);//shmid for the stack descriptor
  ...
  int shmidt=shmget(y,sizeof(int),IPC_CREAT | 0777);//for attachment of a shared array of keys
